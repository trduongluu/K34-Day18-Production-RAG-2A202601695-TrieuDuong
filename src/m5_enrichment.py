...
def _get_client():
    """Tái sử dụng 1 OpenAI client cho cả process (tránh tạo lại mỗi chunk)."""
    global _CLIENT
    if _CLIENT is None:
...
def _chat(system: str, user: str, max_tokens: int, json_mode: bool = False) -> str | None:
    """Gọi 1 lượt chat; trả None nếu không có API key hoặc call lỗi."""
...
def summarize_chunk(text: str) -> str:
    ...
    """
    # Chunk đã ngắn hơn cả một bản tóm tắt → gọi LLM chỉ tốn tiền mà làm dài thêm.
...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    Lý do: user hỏi bằng ngôn ngữ đời thường ("nghỉ cưới mấy ngày?") còn tài liệu
    viết bằng ngôn ngữ hành chính ("nghỉ phép đặc biệt khi kết hôn"). Câu hỏi giả
    định nằm ở "phía user" của khoảng cách từ vựng đó nên kéo hai bên lại gần.
    """
    content = _chat(
    ...
    # Extractive fallback: biến các câu khẳng định thành câu hỏi thô.
...
def contextual_prepend(text: str, document_title: str = "") -> str:
    ...
    Chunk bị cắt rời khỏi tài liệu gốc thường mất tham chiếu ("chính sách này
    áp dụng từ..." — chính sách nào?). Một câu ngữ cảnh dán lên đầu chunk trả
    lại thông tin đó cho cả embedding lẫn BM25.
    """
    content = _chat(
...
_COMBINED_SYSTEM = """Phân tích đoạn văn và trả về JSON:
{
...
def _enrich_single_call(text: str, source: str) -> dict:
    ...
    ⚠️ Cost optimization: 1 API call thay vì 4 calls riêng lẻ → giảm ~75% chi phí
    và ~75% thời gian enrich, với chất lượng gần như tương đương vì cả 4 nhiệm vụ
    đều đọc chung một đoạn văn.
    """
    content = _chat(
        _COMBINED_SYSTEM,
        f"Tài liệu: {source}\n\nĐoạn văn:\n{text}",
